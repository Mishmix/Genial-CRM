      ...
      <div className="max-w-4xl">
        ...
        <div className="card p-5 mb-6 stagger-item">
          <div className="flex items-center gap-6 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              <div className={`w-3 h-3 rounded-full ${settings.llm_provider === 'gemini' ? (settings.gemini_api_key_set ? 'bg-emerald-500' : 'bg-red-500') : (settings.groq_api_key_set ? 'bg-emerald-500' : 'bg-red-500')}`} />
              <span className="text-sm text-[var(--text-secondary)]">Нейросеть ({settings.llm_provider === 'gemini' ? 'Gemini' : 'Groq'})</span>
            </div>
            <div className="flex items-center gap-2">
          ...
          <div className="card p-6 stagger-item">
            ...
            <div className="relative z-10">
              ...
              <div className="flex gap-4 mb-6">
                ...
                <label className={`flex-1 flex items-center justify-center gap-2 p-4 rounded-xl border-2 cursor-pointer transition-all ${settings.llm_provider === 'nim' ? 'border-emerald-500 bg-emerald-500/10' : 'border-[var(--border)] hover:border-[var(--text-muted)]'}`}>
                  <input
                    ...
                    name="llm_provider"
                    value="gemini"
                    checked={settings.llm_provider === 'gemini'}
                    onChange={() => handleSave('llm_provider', 'gemini')}
                    className="hidden"
                  />
                  <span className="font-semibold text-[var(--text-primary)]">Gemini 3 Flash 🌟</span>
                </label>
              </div>
              ...
              {settings.llm_provider === 'gemini' && (
                <div className="animate-in fade-in slide-in-from-top-2 duration-300">
                  <div className="flex items-center gap-2 mb-3">
                    <span className="font-semibold text-[var(--text-primary)]">Google Gemini API ключ</span>
                    {settings.gemini_api_key_set && <span className="badge badge-qualified text-xs">✓ Настроено</span>}
                  </div>
                  <div className="flex gap-3">
                    <div className="flex-1 relative">
                      <input
                        type={showTokens['gemini_api_key'] ? 'text' : 'password'}
                        value={newValues['gemini_api_key'] || ''}
                        onChange={(e) => setNewValues(prev => ({ ...prev, gemini_api_key: e.target.value }))}
                        className="input pr-12 font-mono text-sm"
                        placeholder={settings.gemini_api_key_set ? `Current: ${settings.gemini_api_key}` : "AIza..."}
                      />
                      <button onClick={() => toggleShow('gemini_api_key')} className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors">
                        {showTokens['gemini_api_key'] ? '🙈' : '👁️'}
                      </button>
                    </div>
                    <button
                      onClick={() => handleSave('gemini_api_key', newValues['gemini_api_key'] || '')}
                      disabled={saving === 'gemini_api_key' || !newValues['gemini_api_key']}
                      className="btn btn-primary min-w-[110px]"
                    >
                      {saving === 'gemini_api_key' ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : saved === 'gemini_api_key' ? '✓ Сохранено' : 'Сохранить'}
                    </button>
                  </div>
                  <p className="text-xs text-[var(--text-muted)] mt-2">
                    Используется для быстрой и умной классификации с Gemini 3 Flash. Получить на <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener" className="text-[var(--accent)] hover:underline">aistudio.google.com</a> (бесплатно)
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="flex-1">
                ...
                <ol className="text-sm text-[var(--text-secondary)] space-y-3">
                  {[
                    ...
                    { text: 'Получите API ключ Gemini на', highlight: 'aistudio.google.com', suffix: '(бесплатно)' },
