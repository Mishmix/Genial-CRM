...
Найди ЗАПРОС НА ДИЗАЙН-УСЛУГИ от клиента.

Признаки заказа (должны относиться к дизайну):
- Клиент просит сделать дизайн-работу ("сделай обложку", "нужна превью", "сделай баннер", "хочу лого")
- Указан срок для работы ("завтра", "срочно", "к пятнице")
- Упоминается тип работы (превью, обложка, баннер, лого, оформление канала)

КРИТИЧЕСКИ ВАЖНО:
- Если клиент просит что-то НЕ связанное с дизайном (название видео, совет, просто поздоровался) — ЭТО НЕ ЗАКАЗ (has_order: false).
- В переписке хронология имеет решающее значение!
- Шаг 1: Прочитай переписку СВЕРХУ ВНИЗ.
- Шаг 2: В поле "thought_process" опиши, как менялся заказ. Что просили изначально? Изменилось ли количество или дедлайн в последнем сообщении? Последнее сообщение клиента ПРИОРИТЕТНЕЕ всех предыдущих!
- Шаг 3: Выведи итоговый результат. Если количество или срок изменились в конце, используй именно новые, ОБНОВЛЁННЫЕ значения.
- Если клиент что-то просит (по теме дизайна) = это ЗАКАЗ. Не жди подтверждения дизайнера!
