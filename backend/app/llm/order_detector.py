...
async def detect_order(messages: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    """
...
def parse_deadline(date_str: Optional[str], text: Optional[str]) -> Optional[str]:
    ...
    # Сегодня
    if any(w in text_lower for w in ["сегодня", "today", "hoy", "сьогодні"]):
        return today.strftime("%Y-%m-%d")
    
    # Завтра
    if any(w in text_lower for w in ["завтра", "tomorrow", "mañana", "завтра"]):
