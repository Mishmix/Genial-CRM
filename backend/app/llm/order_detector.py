...
ТВОЯ ЗАДАЧА — максимально точно детектить заказы. Лучше ошибиться в сторону создания заказа, чем пропустить.

КОГДА has_order=true (ОБЯЗАТЕЛЬНО создавай заказ):
1. Клиент упомянул любую дизайн-услугу (превью/обложка/миниатюра/thumbnail/баннер/лого/оформление) — даже кратко, даже в контексте вопроса. Примеры: "нужна обложка", "нужно превью на завтра", "сделаешь превьюшку?", "сколько за обложку?", "нужна одна обложка"
2. Дизайнер (direction=out, "Дизайнер:") в ОТВЕТ написал что угодно утвердительное или деловое — значит он принял запрос. Примеры: "сделаю", "ок", "хорошо", "да", "давай", "принял", "конечно", "окей", "уже делаю", "скинь ТЗ", "к какому сроку?", "цена 20$"
3. Постоянный клиент пишет "нужна ещё обложка"/"ещё одну" — это заказ.
4. Клиент прислал ТЗ, ссылки, референсы, детали видео — это заказ.

КОГДА is_confirmed=true:
- Дизайнер ответил ЛЮБЫМ утвердительным/деловым сообщением на запрос услуги ("сделаю", "ок", "давай", "хорошо", "принял", "конечно", "уже делаю", вопросы по деталям/срокам).
- Клиент явно подтвердил ("да, делаем", "скинул оплату", "приступай", "жду", "ок, делай").
- Согласована цена/количество/дедлайн.
- Клиент — постоянный, просто пишет "нужна ещё" (подразумевает старые условия).
- Обсуждение деталей работы (ссылки, ТЗ) — это всегда подтверждение.

КОГДА is_confirmed=false (но has_order может быть true):
- Клиент только спросил услугу, но дизайнер ещё НЕ ответил в переписке.

КОГДА has_order=false (редко!):
- Клиент НЕ упоминал никаких дизайн-услуг вообще (просто "привет", общий вопрос про оплату/доставку не связанный с дизайном).
- Клиент просит что-то совсем НЕ связанное с дизайном каналов YouTube.

ВАЖНО: Если дизайнер ответил "сделаю" / "ок" / "давай" на запрос услуги — это 100% заказ, is_confirmed=true, confidence=0.95+. Не занижай.
...
Типы услуг:
- превью/обложка/миниатюра/thumbnail/превьюшка → "thumbnail"
- баннер/banner → "banner"
- лого/логотип → "logo"
- оформление канала → "channel_design"

Количество: если не указано = 1. Дедлайн: если не указан = null.

ПРИМЕРЫ (обрати внимание на is_confirmed):

Пример 1:
Клиент: Привет, нужна одна обложка на завтра
Дизайнер: Привет, сделаю
→ {{"has_order":true,"is_confirmed":true,"confidence":0.95,"order":{{"service_type":"thumbnail","quantity":1,"deadline_text":"завтра"}}}}

Пример 2:
Клиент: сколько стоит превью?
Дизайнер: 20$
Клиент: ок, давай сделаем 3 штуки
→ {{"has_order":true,"is_confirmed":true,"confidence":0.98,"order":{{"service_type":"thumbnail","quantity":3,"amount":60}}}}

Пример 3:
Клиент: интересует обложка
(дизайнер не ответил)
→ {{"has_order":true,"is_confirmed":false,"confidence":0.7,"order":{{"service_type":"thumbnail","quantity":1}}}}

Пример 4:
Клиент: Привет
Дизайнер: Здравствуйте
→ {{"has_order":false,"is_confirmed":false,"confidence":0.95}}

Формат ответа — строго JSON, ничего кроме JSON:
{{"thought_process":"короткий анализ","has_order":true,"is_confirmed":true,"confidence":0.95,"reason":"причина","order":{{"service_type":"thumbnail","quantity":1,"amount":null,"deadline_date":"2026-02-27","deadline_text":"завтра","notes":"описание"}}}}"""
...
def _format_existing_orders(existing_orders: Optional[List]) -> str:
    ...
    return (
        "Уже зафиксированный заказ из этой переписки:\n"
        + "\n".join(lines)
        + "\nЕсли клиент в последних сообщениях уточнил количество или срок — верни ОБНОВЛЁННЫЕ значения, а не старые.\n\n"
    )
...
async def detect_order(messages: List[Dict[str, Any]], existing_orders: Optional[List] = None) -> Optional[Dict[str, Any]]:
    ...
    # Форматируем сообщения для GPT (последние 30) с датами
    formatted = []
    for msg in messages[-30:]:
        sender = "Дизайнер" if msg.get("direction") == "out" else "Клиент"
        text = msg.get("text", "")
        sent_at = msg.get("sent_at", "")
        # Показываем дату сообщения чтобы AI корректно вычислял относительные даты
        date_prefix = ""
        if sent_at:
            try:
                dt = datetime.fromisoformat(sent_at.replace("Z", "+00:00"))
                date_prefix = f"[{dt.strftime('%Y-%m-%d %H:%M')}] "
            except (ValueError, AttributeError):
                pass
        if text:
            formatted.append(f"{date_prefix}{sender}: {text}")
